...
const Folder = ({
  ...
  data: FolderProps | null;
  updateTree: (
    newNodeData: { name: string; type: { type: number } },
    parentId: number,
  ) => void;
  renameTreeNode: (newName: string, nodeId: number) => void;
  deleteTreeNode: (nodeId: number) => void;
}) => {

  const [expanded, setExpanded] = useState(false);
  const [addNewComponent, setAddNewComponent] = useState<null | {
    type: number;
  }>(null);
  const [renameComponent, setRenameComponent] = useState<null | {
    nodeId: number;
    newName: string;
  }>(null);
  ...
  useEffect(() => {
    return () => {
      console.log("cleanup happened", data);
    }
  }, []);


  if(data === null) return null;
  const showNewComponent = (type: number, action?: string) => {
    if ((action === undefined) && (type === 1 || type === 2)) {
      setExpanded(true);
      setAddNewComponent({ type });
    } else {
      if(action === "delete") deleteTreeNode(type);
      else handleRename(type);
    }
  };
  ...
  const addNewComponentHandler = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      updateTree(
        { name: e.currentTarget.value, type: addNewComponent! },
        data.id,
      );
      setAddNewComponent(null);
    }
      ...
      <div>
        ...
        {renameComponent === null && (
          <>
            ...
            <Button text="Delete" onClick={showNewComponent} type={data.id} action="delete"/>
          </>
        )}
